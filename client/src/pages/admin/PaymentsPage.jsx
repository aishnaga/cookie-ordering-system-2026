...
import { Box, Heading, VStack, Table, Thead, Tbody, Tr, Th, Td, Button, NumberInput, NumberInputField, Input, HStack, useToast, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalFooter, ModalCloseButton, useDisclosure } from '@chakra-ui/react';
import api from '../../api';
...
export default function PaymentsPage() {
  ...
  const openPayment = async (family) => {
    ...
    setOrders(res.data.filter(o => o.status !== 'pending' && o.status !== 'paid'));
    setAmount(0);
    setNotes('');
    onOpen();
  };
  ...
  const recordPayment = async (orderId) => {
    if (amount <= 0) return;
    await api.post(`/orders/${orderId}/payment`, { amount, notes });
    ...
    <Box p={6}>
        ...
        <Table variant="simple" bg="white">
          ...
          <Tbody>
            {families.map(f => (
              <Tr key={f.id}>
                <Td>{f.name}</Td>
                <Td isNumeric>${f.owed.toFixed(2)}</Td>
                <Td isNumeric>${f.paid.toFixed(2)}</Td>
                <Td isNumeric color={f.balance > 0 ? 'red.500' : 'green.500'}>${Math.abs(f.balance).toFixed(2)}</Td>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack spacing={4}>
              <NumberInput min={0} value={amount} onChange={(_, v) => setAmount(v)} w="100%">
                <NumberInputField placeholder="Amount" />
              </NumberInput>
              <Input placeholder="Notes (e.g., Venmo, Cash)" value={notes} onChange={e => setNotes(e.target.value)} />
              {orders.map(o => (
                <Button key={o.id} w="100%" onClick={() => recordPayment(o.id)}>
                  Apply to Order #{o.id} (${(o.amount_owed - o.amount_paid).toFixed(2)} remaining)
                </Button>
              ))}
            </VStack>
          </ModalBody>
